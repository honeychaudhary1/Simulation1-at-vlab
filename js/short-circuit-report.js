...
function readReportData() {
    const rawData = localStorage.getItem(SHORT_CIRCUIT_REPORT_STORAGE_KEY);
...
function renderEmptyState() {
    ...
    if (fieldIds.conclusionText) {
        fieldIds.conclusionText.textContent = "Generate the report from the short circuit experiment page after adding the observation reading.";
    }

    if (fieldIds.summaryText) {
        fieldIds.summaryText.textContent = "This report page was opened without a saved short circuit report payload.";
    }
}
...
function downloadReportHtml() {
    ...
    link.href = url;
    link.download = "short-circuit-report.html";
